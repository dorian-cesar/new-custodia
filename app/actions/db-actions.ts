...
const initDbAndFetch = async () => {
  ...
  const needsReseed = lockerCount !== 250 || (await LockerModel.findOne({ where: { label: null } }));
  if (needsReseed) {
    // Recreate/reseed all lockers to ensure we have exactly 250 lockers with correct sizes and labels
