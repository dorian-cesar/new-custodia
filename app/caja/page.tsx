    ...
    <div className="min-h-screen bg-background">
              ...
              <TableBody>
                {paginatedRegisters.length === 0 ? (
                  ...
                  paginatedRegisters.map((register) => {
                    ...
                    const ingresosTarjeta = regTxs.filter(t => t.type === 'income' && t.description.includes('Tarjeta')).reduce((s, t) => s + t.amount, 0)
                    const ingresosEfectivo = regTxs.filter(t => t.type === 'income' && !t.description.includes('Tarjeta')).reduce((s, t) => s + t.amount, 0)
                    const gastosEfectivo = regTxs.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0)
                    ...
                    <TableRow key={register.id} className="border-border">
                      ...
                      <TableCell className="text-right">
                        <div className="flex flex-col items-end">
                          <span className="text-amber-500 font-medium text-xs">Efectivo: Gs. {ingresosEfectivo.toLocaleString('es-PY')}</span>
                          <span className="text-blue-500 font-medium text-xs">Tarjeta: Gs. {ingresosTarjeta.toLocaleString('es-PY')}</span>
                        </div>
                      </TableCell>
                      <TableCell className="text-destructive text-right font-medium">
                        Gs. {gastosEfectivo.toLocaleString('es-PY')}
                      </TableCell>
                      <TableCell className="text-accent text-right font-medium">
                        Gs. {saldoEsperadoEfectivo.toLocaleString('es-PY')}
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        {diferenciaCaja !== null ? (
                          <span className={`text-xs font-bold ${diferenciaCaja === 0 ? 'text-emerald-500' : diferenciaCaja > 0 ? 'text-blue-500' : 'text-destructive'}`}>
                            {diferenciaCaja === 0 ? 'Cuadrada ✓' : diferenciaCaja > 0 ? `Sobrante: +Gs. ${diferenciaCaja.toLocaleString('es-PY')}` : `Faltante: -Gs. ${Math.abs(diferenciaCaja).toLocaleString('es-PY')}`}
                          </span>
                        ) : (
        ...
        <DialogContent className="bg-card border-border">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                type="text"
                value={openingAmount ? Number(openingAmount).toLocaleString('es-PY') : ''}
        ...
        <DialogContent className="bg-card border-border">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                type="text"
                value={closingAmount ? Number(closingAmount).toLocaleString('es-PY') : ''}
      ...
      <Dialog open={showGiroDialog} onOpenChange={setShowGiroDialog}>
        <DialogContent className="bg-card border-border">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label>Monto a Retirar (Gs.)</Label>
              <Input
                type="text"
                value={giroAmount ? Number(giroAmount).toLocaleString('es-PY') : ''}
            ...
            <div className="space-y-4 py-4">
              <div className="bg-muted/50 p-4 rounded-lg space-y-3">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-medium">Gs. {closeSummary.expected.toLocaleString('es-PY')}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-muted-foreground">Cierre de Caja (Declarado)</span>
                  <span className="font-medium">Gs. {closeSummary.declared.toLocaleString('es-PY')}</span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`font-bold ${closeSummary.difference === 0 ? 'text-emerald-500' : closeSummary.difference > 0 ? 'text-blue-500' : 'text-destructive'}`}>
                    {closeSummary.difference === 0 
                      ? 'Cuadrada ✓' 
                      : closeSummary.difference > 0 
                        ? `Sobrante: +Gs. ${closeSummary.difference.toLocaleString('es-PY')}` 
                        : `Faltante: -Gs. ${Math.abs(closeSummary.difference).toLocaleString('es-PY')}`
                    }
                  </span>
                </div>
              </div>
