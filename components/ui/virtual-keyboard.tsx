        ...
        <div className="flex flex-col gap-1.5 max-w-[800px] mx-auto w-full">
          {qwertyRows.map((row, rowIndex) => (
            <div key={rowIndex} className="flex justify-center gap-1.5 w-full">
              {row.map((key) => {
                let buttonClass =
                  "flex-1 h-11 min-w-[28px] max-w-[64px] rounded-lg font-bold text-sm bg-zinc-800 hover:bg-zinc-700 text-white flex items-center justify-center border border-zinc-700 transition-all active:scale-95 cursor-pointer";
                ...
                if (key === "SHIFT") {
                  ...
                  buttonClass =
                    "flex-1 h-11 max-w-[80px] rounded-lg bg-zinc-700 hover:bg-zinc-600 text-white flex items-center justify-center border border-zinc-600 transition-all active:scale-95 cursor-pointer";
                  content = <Delete className="h-4.5 w-4.5" />;
                } else if (key === "SPACE") {
                  buttonClass =
                    "flex-[3] h-11 min-w-[120px] max-w-[320px] rounded-lg bg-zinc-800 hover:bg-zinc-700 text-white flex items-center justify-center border border-zinc-700 transition-all active:scale-95 cursor-pointer";
                  content = <Space className="h-4.5 w-4.5" />;
                } else if (key === "DONE") {
                  buttonClass =
                    "flex-1 h-11 max-w-[80px] rounded-lg bg-emerald-600 hover:bg-emerald-500 border border-emerald-500 font-extrabold text-xs text-white flex items-center justify-center transition-all active:scale-95 cursor-pointer";
                  content = "LISTO";
                } else if (key === "LAYOUT_TOGGLE") {
                  buttonClass =
                    "flex-1 h-11 max-w-[80px] rounded-lg bg-zinc-700 hover:bg-zinc-600 border border-zinc-600 font-extrabold text-xs text-white flex items-center justify-center transition-all active:scale-95 cursor-pointer";
                  content = "123";
                } else if (key === "SYM" || key === "ABC") {
                  buttonClass =
                    "flex-1 h-11 max-w-[64px] rounded-lg bg-zinc-700 hover:bg-zinc-600 border border-zinc-600 font-extrabold text-xs text-white flex items-center justify-center transition-all active:scale-95 cursor-pointer";
                  content = key;
                }
        ...
        <div className="flex flex-col gap-1.5 max-w-[280px] mx-auto w-full py-1">
          {numericKeys.map((row, rowIndex) => (
            <div key={rowIndex} className="flex justify-center gap-1.5 w-full">
              {row.map((key) => {
                let buttonClass =
                  "flex-1 h-12 rounded-lg font-black text-base bg-zinc-800 hover:bg-zinc-700 text-white flex items-center justify-center border border-zinc-700 transition-all active:scale-95 cursor-pointer";
                let content: React.ReactNode = key;

                if (key === "BACKSPACE") {
                  buttonClass =
                    "flex-1 h-12 rounded-lg bg-zinc-700 hover:bg-zinc-600 text-white flex items-center justify-center border border-zinc-600 transition-all active:scale-95 cursor-pointer";
                  content = <Delete className="h-5 w-5" />;
                } else if (key === "DONE") {
                  buttonClass =
                    "flex-[1.5] h-12 rounded-lg bg-emerald-600 hover:bg-emerald-500 border border-emerald-500 font-extrabold text-sm text-white flex items-center justify-center transition-all active:scale-95 cursor-pointer";
                  content = "LISTO";
                } else if (key === "LAYOUT_TOGGLE") {
                  buttonClass =
                    "flex-[1.5] h-12 rounded-lg bg-zinc-700 hover:bg-zinc-600 border border-zinc-600 font-extrabold text-sm text-white flex items-center justify-center transition-all active:scale-95 cursor-pointer";
                  content = "ABC";
                }
